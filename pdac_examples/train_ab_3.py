...
from pl_modules.pdac_module_ab3 import PDACModuleAB3
...
def build_args():
    ...
    parser.add_argument(
        "--config_file",
        default="pdac_examples/config/stanford2d/ab_3.yaml",
        type=pathlib.Path,
    ...
    parser = PDACModuleAB3.add_model_specific_args(parser)
    parser.set_defaults(
        num_cascades=8,
        pools=4,
        chans=18,
...
def cli_main(args):
    validate_single_gpu_runtime(args, use_compile=args.use_compile, context="train_ab_3.py")
    if args.use_compile and uses_activation_checkpointing(args):
        raise ValueError(
            "train_ab_3.py does not support `use_compile=true` together with "

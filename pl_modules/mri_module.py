...
from single_gpu_runtime import uses_activation_checkpointing

import fastmri
from fastmri import evaluate
from torchmetrics.metric import Metric
from checkpoint_utils import sanitize_state_dict_keys
...
class MriModule(pl.LightningModule):
    ...
    def _apply_compile_if_requested(self, use_compile=None):
        if use_compile is None:
            use_compile = getattr(self, "use_compile", False)
        if not use_compile:
            return self
        if self._model_is_compiled:
            return self
        if not hasattr(torch, "compile"):
            raise RuntimeError("torch.compile is not available in this PyTorch build.")
        if getattr(self, "model", None) is None:
            raise RuntimeError("Expected the LightningModule to expose a `model` attribute.")
        if uses_activation_checkpointing(self.hparams) or uses_activation_checkpointing(self.model):
            raise RuntimeError(
                "torch.compile is incompatible with activation checkpointing in the current runtime. "
                "Disable `use_checkpointing` / `use_checkpoint` before enabling compile."
            )

        compile_targets = self._get_compile_targets()
        for parent_module, attr_name in compile_targets:
            target = getattr(parent_module, attr_name)
            setattr(
                parent_module,
                attr_name,
                torch.compile(target, mode="reduce-overhead"),
            )
        self._model_is_compiled = True
        return self

    def _get_compile_targets(self):
        if hasattr(self.model, "net") and hasattr(self.model.net, "model"):
            return [(self.model.net, "model")]
        if hasattr(self.model, "cascades"):
            return [(cascade, "model") for cascade in self.model.cascades]
        raise RuntimeError(
            "Unsupported model structure for torch.compile. "
            "Expected singlecoil `model.net.model` or multicoil `model.cascades[*].model`."
        )

    def _evaluation_autocast_context(self):
    ...
    def _get_num_validation_batches(self) -> int:
        ...
        return int(num_val_batches)

    def on_validation_epoch_start(self):
        self._validation_step_outputs = []
        if self.val_log_indices is None:
            num_val_batches = max(self._get_num_validation_batches(), 1)
            num_log_images = min(self.num_log_images, num_val_batches)
            self.val_log_indices = list(
                np.random.permutation(num_val_batches)[:num_log_images]
            )

    def _prepare_validation_outputs(self, val_logs):
        # check inputs
        for k in (
            "batch_idx",
            "fname",
    ...
    def on_validation_batch_end(
        ...
        if outputs is None:
            return
        self._validation_step_outputs.append(self._prepare_validation_outputs(outputs))
    ...
    def on_validation_epoch_end(self):
        val_logs = self._validation_step_outputs
        if not val_logs:
            return
        # aggregate losses
        losses = []
        ...
        self.log("validation_loss", val_loss / tot_slice_examples, on_epoch=True)
    ...
    def on_test_epoch_end(self):
        test_logs = self._test_step_outputs
        if not test_logs:
            return

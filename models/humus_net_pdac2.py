...
class HUMUSNet_pdac(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #     ]
        # )
        # im_size = kwargs.pop('img_size', None)
        ...
        # mlp_ratio = kwargs.pop('mlp_ratio', None)


        self.cascades = nn.ModuleList(
            [
                VarNetBlock(
                    Warp(img_channels=2*self.num_adj_slices, **kwargs), self.num_adj_slices, im_size) for _ in range(num_cascades)
            ]
        )
        # self.cascades = nn.ModuleList(
        #     [

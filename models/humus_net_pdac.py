...
# from models.nafnet2 import Warp
# from models.dy_naf_warp import Warp
        ...
        for i, cascade in enumerate(self.cascades):
            if self.use_checkpoint:
                kspace_pred = checkpoint.checkpoint(
                    cascade, kspace_pred, masked_kspace, mask, sens_maps, current_prob
                )
            else:
                kspace_pred = cascade(
                    kspace_pred, masked_kspace, mask, sens_maps, current_prob
                )
            ...
            current_mask = build_effective_center_mask(
                ...
                kspace_pred.shape[-3],
                kspace_pred.shape[-2],
                self.progressive_mask_schedule[i],
            ...
            # Keep the historical multicoil mask shape for downstream callers.
...
class VarNetBlock(nn.Module):
    ...
    def forward(
        ...
        restore_im = self.apply_model(
            self.sens_reduce(current_kspace, sens_maps), current_prob
        )
